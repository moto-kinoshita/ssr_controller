        ...
        while self.running:
            ...
            if buff_waiting > 0:
                ...
                for idx in self.tc_index:
                    if True: #debug print
                      if idx == 1:
                         print("debug_print{210104}  ",
                         "time=",temperatures[idx],
                         "second=",temperatures[1][temperatures[1].rfind(':')+1:]
                         )
                    self.tc_queue_dict[idx].append(temperatures[idx])      
                    self.tc_now = temperatures[idx]     #OBS! {210104} need time_in_sseconds(for dt calculation)
